...
for batch in batch_sizes:
    ...
    for mode in ("standard", "tcl"):
        print(f"\n--- {mode.upper()} mode ---")
        cmd_template = standard_cmd if mode == "standard" else tcl_cmd
        cwd = workdirs[mode]
        batch_times = []
...
output_path = os.path.join(output_dir, f"profile_tcl_{timestamp}.png")
plt.savefig(output_path)

print(f"Plot saved to {output_path}")

csv_output_path = os.path.join(output_dir, f"profile_tcl_{timestamp}.csv")
with open(csv_output_path, mode="w", newline="") as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(["Batch Size", "standard Time (s)", "tcl Time (s)"])
    for batch, par_time, ser_time in zip(
        batch_sizes, results["standard"], results["tcl"]
    ):
