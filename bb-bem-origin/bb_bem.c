...
double element_ij_(int *i, int *j, int *nond, int *nofc, struct coordinate *np,
                   int *face2node);
void pbicgstab(int dim, double **mat, double *rhs, double *sol, double tor,
               int max_steps);
...
int main() {
  ...
  /* Read number of integer parameters set on each face from input data file :
   * nint_para_fc  */
  fscanf(fp, "%d", &nint_para_fc);

  /* Read number of real(double precision) parameters set on each face from
   * input data file : ndble_para_fc  */
...
void pbicgstab(int dim, double **mat, double *rhs, double *sol, double tor,
  ...
  for (step = 1; step <= max_steps; step++) {
    ...
    for (i = 0; i < dim; i++) {
      sol[i] = sol[i] + alpha * kp[i] + zeta * kt[i];
    }
    for (i = 0; i < dim; i++) {
      r[i] = t[i] - zeta * akt[i];
    }
